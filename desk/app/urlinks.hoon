...
?-  -.act
  ...
  new-state
    %delete-link
  =/  new-state  state(links (oust [index.act 1] links))
  new-state
    %edit-link
  =/  new-links  (snap links.state index.act [label.act url.act])
...
++  action-parser
  ...
  ?:  (~(has by args) 'send-delete')
    [%delete-link `@ud`(slav %ud (~(got by args) 'index'))]
  ?:  (~(has by args) 'edit-link')
    [%edit-link `@ud`(slav %ud (~(got by args) 'index')) (~(got by args) 'label') (~(got by args) 'url')]
...
?+    method.request.inbound-request  
  ...
  ?+    site.request-line  
    ...
    ::
      [%apps %urlinks %urlinks ~]
    :_  state
    (send [200 ~ [%text-javascript urlinks-js]])
    ::
      [%apps %urlinks %htmx ~]
    :_  state
    ...
    ?.  authenticated.inbound-request
      :_  state
      (send [200 ~ [%manx (page body-public)]])
    :_  state
    ...
    (send [200 ~ [%manx link-maker]])
      [%apps %urlinks %link-maker-settings ~]
    ?.  authenticated.inbound-request
      :_  state 
      (send [404 ~ [%plain "404 - Not Found"]])
    :_  state
    (send [200 ~ [%manx body-admin-settings]])
      [%apps %urlinks %link-maker-settings2 ~]
    ?.  authenticated.inbound-request
      :_  state 
      (send [404 ~ [%plain "404 - Not Found"]])
    :_  state
    (send [200 ~ [%manx (body-admin2 state)]])
  ==
    %'POST'
  ...
  ?+    site.request-line  
    ...
    :_  new-state
      (send [200 ~ [%manx (render-last-link new-state)]])
    ::
      [%apps %urlinks %link-maker-delete ~]
    ?.  authenticated.inbound-request
      :_  state 
      (send [404 ~ [%plain "404 - Not Found"]])
    :_  state
      (send [202 ~ %json [%o p=[n=[p='message' q=[%s p='Post request successful']] l=~ r=~]]])
      [%apps %urlinks %link-maker-settings-delete ~]
    ?.  authenticated.inbound-request
      :_  state 
      (send [404 ~ [%plain "404 - Not Found"]])
    ?~  action-parsed  `state
    ...
    =/  new-state=_state  (action-handler2 `action`action-parsed)
    :_  new-state
      (send [200 ~ [%manx (body-admin2 new-state)]])
    ::
    ::
  ==
==
...
++  settings-icon
;svg
  =hx-get  "urlinks/link-maker-settings"
  =hx-target  "body"
  =hx-swap  "innerHTML"
  =width  "32px"
  =height  "32px"
...
++  settings-icon2
;svg
  =hx-post  "urlinks/link-maker-settings2"
  =hx-target  "body"
...
;div(class "link-maker")
  ;button(class "delete-button", type "submit", hx-trigger "click", hx-post "urlinks/link-maker-delete", name "link-maker-delete-no-update", value "0", hx-target "closest .link-maker", hx-swap "delete")
    ;+  delete-icon
  ==
  ;div.inputs
  ...
  ==
  ;button(class "add-button", type "submit", hx-post "urlinks/link-maker", hx-include "closest .link-maker", name "send-add-link", value "send-add-link", hx-target "closest .link-maker", hx-swap "outerHTML")
    ;+  check-icon
  ==
==

++  links-to-link-makers  
^-  marl
=/  counter=@ud  0
=|  marl-object=marl  
|-
...
marl-object
%=  $
marl-object  (snoc marl-object (manx-link-maker-object counter))
links  t.links
counter  +(counter)
==

++  manx-link-maker-object
|=  q=@ud
=/  link-obj=link  (snag 0 links.state) 
~&  >  link-obj
^-  manx
;div(class "link-maker")
  ;button(class "delete-button", type "submit", hx-post "urlinks/link-maker-settings-delete", hx-include "closest .link-maker", name "send-delete", value "send-delete", hx-target "closest .link-maker", hx-swap "delete")
    ;+  delete-icon
  ==
  ;div.inputs
    ;input(class "link-name", type "text", name "label", value "{(trip label.link-obj)}", placeholder "Name"); :: why does my code work? idk y this works
    ;input(class "link-url", type "text", name "url", value "{(trip url.link-obj)}", placeholder "URL");
    ;input(type "hidden", name "index", value "{<q>}");
  ==
  ;button(class "add-button", type "submit", hx-post "urlinks/link-maker-edit", hx-include "closest .link-maker", name "edit-link", value "edit-link", hx-target "closest .link-maker", hx-swap "delete")
    ;+  check-icon
  ==
==
...
marl-object
%=  $
marl-object  (snoc marl-object (manx-link-object [i.links counter]))
links  t.links
counter  +(counter)
==
...
|=  [p=link q=@ud]
^-  manx
;a(href "{(trip url.p)}", target "_blank")
  ;div.link
    ; {(trip label.p)}
    ;input(type "hidden", name "index", value "{<q>}");
  ==
...
;div.content-container
  ;header
    ;div.header-buttons;
    ;+  (profile-image state)
    ;div.profile-name: {(trip ?:(=(name *@t) `@t`(scot %p our.bowl) name))}
    ;div(class "bio"): {(trip bio)}
  ==
  ;div.link-container
    ;*  render-list
...
;div.content-container
  ;header
    ...
    ==
    ;+  (profile-image state)
    ;div.profile-name: {(trip ?:(=(name.state *@t) `@t`(scot %p our.bowl) name.state))}
...
;form(id "settings-form")
  ;div
    ;label: Name  
    ;input(type "text", name "name", value "{(trip ?:(=(name *@t) `@t`(scot %p our.bowl) name))}");
    ;label: Image 
